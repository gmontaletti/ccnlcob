...
test_that("tipologie_contrattuali ha la struttura documentata", {
  ...
  expect_identical(
    names(tipologie),
    c("cod_tipologia_contrattuale", "des_tipologia_contrattuale",
...
test_that("macro_tipologia è sempre nell'insieme ammesso", {
  ...
  expect_true(all(c("Tempo indeterminato", "Tempo determinato", "Apprendistato",
    "Somministrazione", "Intermittente", "Collaborazioni", "Tirocinio",
    "Domestico") %in% tipologie$macro_tipologia))
})
...
test_that("i codici principali hanno la macro-tipologia attesa", {
  macro <- function(cod) tipologie[cod_tipologia_contrattuale == cod, macro_tipologia]
...
test_that("cob_esempio ha le colonne del contratto dati e circa 5000 righe", {
  ...
  expect_identical(
    names(cob),
    c("id", "cf", "inizio", "fine", "codice_cnel", "ccnl",
...
test_that("cob_esempio contiene NA, sentinelle e sovrapposizioni", {
  ...
  expect_true(all(cob$cod_tipologia_contrattuale %in% tipologie$cod_tipologia_contrattuale))
})
